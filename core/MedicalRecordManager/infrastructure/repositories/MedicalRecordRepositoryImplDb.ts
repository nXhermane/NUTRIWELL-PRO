...
import { AggregateDomainEvent, AggregateID, Mapper } from "@shared";
...
export class MedicalRecordRepositoryImpl implements MedicalRecordRepository {
   ...
   async save(medicalRecord: MedicalRecord): Promise<void> {
      ...
      try {
       ...
       AggregateDomainEvent.get().dispatchEventsForMarkedAggregate(medicalRecord.id)
      } catch (e: any) {
   ...
   async delete(medicalRecordId: AggregateID): Promise<void> {
     ...
     AggregateDomainEvent.get().dispatchEventsForMarkedAggregate(medicalRecordId)    
      } catch (error: any) {
