...
export class StandardMedicalConditionMapper
   implements Mapper<StandardMedicalCondition, StandardMedicalConditionPersistenceType, StandardMedicalConditionDto>
{
   ...
   toDomain(record: StandardMedicalConditionPersistenceType): StandardMedicalCondition {
      const recommendationsResult = record.recommendations.map((recommendation: NeedsRecommendationDto) =>
         NeedsRecommendationFactory.create(recommendation),
      );
