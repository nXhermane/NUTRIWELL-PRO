...
export class StandardMedicalConditionRepositoryImpl implements StandardMedicalConditionRepository {
   ...
   async save(medicalCondition: StandardMedicalCondition): Promise<void> {
      try {
         ...
         if (exist) {
            await this.db
               .update(standardMedicalConditions)
               .set(medicalConditionPersistence)
               .where(eq(standardMedicalConditions, medicalConditionPersistence.id));
         } else {
   ...
   async getById(medicalConditionId: AggregateID): Promise<StandardMedicalCondition> {
      try {
         const rowMedicalCondition = await this.db
            .select()
            .from(standardMedicalConditions)
            .where(eq(standardMedicalConditions, medicalConditionId as string))
            .get();
   ...
   private async checkIfExist(medicalConditionId: AggregateID): Promise<boolean> {
      const medCond = await this.db
         .select()
         .from(standardMedicalConditions)
         .where(eq(standardMedicalConditions.id, medicalConditionId as string));
      return !!medCond;
   }
}
