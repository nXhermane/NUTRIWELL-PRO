...
export class StandardObjective extends AggregateRoot<IStandardObjective> {
   ...
   addRecommendations(...recommendations: NeedsRecommendation[]): void {
      ...
      this.validate();
      this.addDomainEvent(
         new ObjectiveRecommendationAddedEvent({
            objectiveId: this.id,
   ...
   removeRecommendation(...recommendations: NeedsRecommendation[]) {
      ...
      this.validate();
      this.addDomainEvent(
         new ObjectiveRecommendationRemovedEvent({
            objectiveId: this.id,
            recommendations: recommendations,
         }),
      );
   ...
   public validate(): void {
      ...
      if (this.props.type === ObjectiveType.Measure) {
         ...
         if (!this.props.initialValue || Guard.isNegative(this.props.initialValue as number).succeeded)
            throw new NegativeValueError("La valeur initial standard ne doit être négative.");
         if (!this.props.targetValue || Guard.isNegative(this.props.targetValue as number).succeeded)
            throw new NegativeValueError("La valeur a atteindre standard ne doit être négative.");
      }
      this._isValid = true;
   }
