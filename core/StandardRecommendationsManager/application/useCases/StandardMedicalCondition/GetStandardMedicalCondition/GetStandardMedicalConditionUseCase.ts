...
import { StandardMedicalConditionDto, StandardMedicalConditionError, StandardMedicalConditionPersistenceType, StandardMedicalConditionRepository } from "@/core/StandardRecommendationsManager/infrastructure";
...
export class GetStandardMedicalConditionUseCase implements UseCase<GetStandardMedicalConditionRequest, GetStandardMedicalConditionResponse> {
    constructor(private repo: StandardMedicalConditionRepository, private mapper: Mapper<StandardMedicalCondition, StandardMedicalConditionPersistenceType, StandardMedicalConditionDto>) { }
    async execute(request: GetStandardMedicalConditionRequest): Promise<GetStandardMedicalConditionResponse> {
        try {
            if (request.standardMedicalConditionId) {
                ...
                const standardMedicalConditionDtos = standardMedicalConditions.map(standardMedicalCondition => this.mapper.toResponse(standardMedicalCondition))
                return right(Result.ok<StandardMedicalConditionDto[]>(standardMedicalConditionDtos))
            }
        } catch (error) {
            if (error instanceof StandardMedicalConditionError) return left(new GetStandardMedicalConditionErrors.MedicalConditionNotFoundError(error, request.standardMedicalConditionId))
