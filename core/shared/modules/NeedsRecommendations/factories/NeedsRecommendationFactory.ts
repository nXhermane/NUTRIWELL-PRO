...
export class NeedsRecommendationFactory {
   ...
   private static getRecommendation(props: INeedsRecommendation<any>): NeedsRecommendation {
      switch (props.type) {
         case "Interval":
            return new NeedsRecommendationByInterval(props);
            break;
         case "Exclusion":
            return new NeedsRecommendationByExclusion(props);
            break;
         case "Formular":
            return new NeedsRecommendationByFormular(props);
            break;
         case "Relativity":
            return new NeedsRecommendationByRelativity(props);
            break;
         case "Substitution":
            return new NeedsRecommendationBySubstitution(props);
            break;
         default:
            throw new ArgumentOutOfRangeException("this recommendation is not supported.");
      }
   }
}
