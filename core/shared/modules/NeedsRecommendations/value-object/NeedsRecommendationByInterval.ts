...
export class NeedsRecommendationByInterval extends NeedsRecommendation<INeedsRecommendationByInterval> {
   ...
   protected validate(props: INeedsRecommendation<INeedsRecommendationByInterval>): void {
      ...
      if (!Guard.isNumber(props.data.max).succeeded && !Guard.isNumber(props.data.min).succeeded)
         throw new InvalidArgumentFormatError("Pour la recommandation par intervalle, au moins une des bornes de l'intervalle doit être donnée.");
   }
   isMax(): boolean {
