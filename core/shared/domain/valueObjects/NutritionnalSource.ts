...
export class NutritionalSource extends ValueObject<INutritionalSource> {
    protected validate(props: INutritionalSource): void {
        if (Guard.isEmpty(props.organization).succeeded) throw new EmptyStringError("L'organisation doit être fourni dans lors de la definition du source nutritionnelle.")
        if (Guard.isEmpty(props.country).succeeded) throw new EmptyStringError("Le pays d'origine doit être fourni dans lors de la definition du source nutritionnelle.")
        if (Guard.isNegative(props.publicationYear).succeeded) throw new NegativeValueError("L'année de publication doit être un nombre positif dans lors de la definition du source nutritionnelle.")
    }
