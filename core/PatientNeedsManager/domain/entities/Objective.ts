import { AggregateID, ArgumentInvalidException, CDate, EmptyStringError, Entity, ExceptionBase, Guard, INeedsRecommendation, ITimeframe, NeedsRecommendation, NegativeValueError, ObjectiveStatus, ObjectiveType, Result, Timeframe } from "@/core/shared";
...
export class Objective extends Entity<IObjective> {
    ...
    getValueProgression(): number {
        ...
        const progress = Math.abs(this.props.initialValue - this.props.currentValue) * 100 / Math.abs(this.props.targetValue - this.props.initialValue)
    ...
    public validate(): void {
        ...
        if (this.props.type === ObjectiveType.Measure) {
            ...
            if (!this.props.initialValue || Guard.isNegative(this.props.initialValue as number).succeeded)
                throw new NegativeValueError("La valeur initial ne doit être négative ou doit être fournir si vous utilisez un objectif de type 'Measure'.");
            if (!this.props.targetValue || Guard.isNegative(this.props.targetValue as number).succeeded)
                throw new NegativeValueError("La valeur a atteindre ne doit être négative ou doit être fournir si vous utilisez un objectif de type 'Measure'.");
            if (!this.props.currentValue || Guard.isNegative(this.props.currentValue).succeeded) throw new NegativeValueError("La valeur actuelle ne doit être négative ou doit être fournir si vous utilisez un objectif de type 'Measure'.")
        }
        this.updateStatusBasedOnProgress()
        this._isValid = true;
    }
