...
export class NutritionalVariable extends ValueObject<INutritionalVariable> {
    protected validate(props: INutritionalVariable): void {
        if (Guard.againstNullOrUndefined(props.source, "NutritionalSource").succeeded) throw new EmptyStringError("Le source de la variable nutritionnelle ne peut pas être null.");
    }
