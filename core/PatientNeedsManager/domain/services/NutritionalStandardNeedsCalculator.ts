...
export class NutritionalStandardNeedsCalculator implements INutritionalStandardNeedsCalculator {
   ...
   async generatePatientNeeds(patientProfil: PatientProfil, patientNeedsModel: PatientNeedsModel, additionalContext: { [key: string]: any } = {}): Promise<PatientNeeds> {
      ...
      const energyAdditionalContext = Object.fromEntries(Object.values(energyNeedsValue).map(nutrientNeedsValue => [nutrientNeedsValue.tagname, nutrientNeedsValue.value]))
      const macronutrientsNeedsValue = await this.calculateMacronutrientProportion(patientProfil, patientNeedsModelProps.macronutrients, { ...additionalContext, ...energyAdditionalContext });
      const micronutrientsNeedsValue = await this.calculateMicronutrientProportion(patientProfil, patientNeedsModelProps.micronutrients, { ...additionalContext, ...energyAdditionalContext });
      ...
      if (patientNeedsResult.isFailure) throw new NutritionalStandardNeedsCalculatorError(`Erreur lors de la génération des besoins alimentaires : ${patientNeedsResult.err}`);
   ...
   private async calculateMicronutrientProportion(
      ...
      for (const [key, value] of Object.entries(micronutrientsModel)) {
         micronutrientsNeedsValue[key] = await this.calculateNutrientNeedsFormNutrientNeedsCalculatationModel(value, patientProfil, additionalContext);
      }
   ...
   private async calculateMacronutrientProportion(
      ...
      for (const [key, value] of Object.entries(macronutrientsModel)) {
         macroNutrientsNeedsValue[key] = await this.calculateNutrientNeedsFormNutrientNeedsCalculatationModel(value, patientProfil, additionalContext);
      }
