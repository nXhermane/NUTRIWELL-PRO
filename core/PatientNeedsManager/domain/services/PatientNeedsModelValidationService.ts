   ...
   async validate(patientNeedsModel: PatientNeedsModel): Promise<boolean> {
      try {
         ...
         for (const rule of patientNeedsModelValidationRules) {
            const insolvedNutrients = Object.fromEntries(
               rule.nutrientsInsolved.map((nutrientTagname) => [nutrientTagname.toString(), nutrientObject[nutrientTagname.toString()].value]),
            );
            ...
            if (expressionEvaluationResult != EvaluateMathExpression.ConditionResult.True)
               throw new PatientNeedsModelValidationServiceErrors("Erreur lors de la validation du PatientNeedsModel.", new Error(), { rule });
         }
         patientNeedsModel.isValidModel = true;
         return true;
      } catch (error) {
         throw new PatientNeedsModelValidationServiceErrors("An error occurred during validation.", error as Error);
      }
   }
}
