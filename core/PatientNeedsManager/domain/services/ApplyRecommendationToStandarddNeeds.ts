...
export class ApplyRecommendationToStandarddNeeds implements IApplyRecommendationToStandardNeeds {
    constructor(private dataComposerService: IDataComposerService, private needsRecommendatonPriorityManager: INeedsRecommendationPriorityManagerService) { }
    async apply(patientNeeds: PatientNeeds, patientProfil: PatientProfil, applyMedicalConditionFirst: boolean = false): Promise<Result<PatientNeeds>> {
        try {
            ...
            if (applyMedicalConditionFirst) {
                const patientNeedsModifyByMedicalCondition = await this.applyMedicalConditionRecommendationToPatientNeeds(patientNeeds, medicalConditions)
                ...
                const patientNeedsModifyByObjective = await this.applyObjectiveRecommendationToPatientNeeds(patientNeedsModifyByMedicalCondition.val, objectives)
                ...
                if (patientNeedsModifyByObjective.isFailure) return Result.fail<PatientNeeds>(String(patientNeedsModifyByObjective.err))
                const patientNeedsModifyByMedicalCondition = await this.applyMedicalConditionRecommendationToPatientNeeds(patientNeedsModifyByObjective.val, medicalConditions)
                if (patientNeedsModifyByMedicalCondition.isFailure) return Result.fail<PatientNeeds>(String(patientNeedsModifyByMedicalCondition.err))
                return Result.ok<PatientNeeds>(patientNeedsModifyByObjective.val)
            }
        } catch (error) {
    ...
    async applyMedicalConditionRecommendationToPatientNeeds(patientNeeds: PatientNeeds, medicalConditions: MedicalCondition[]): Promise<Result<PatientNeeds>> {
        try {
            ...
            const [energy, macronutrients, micronutrients] = await Promise.all([
                ...
                this.applyRecommendationToNutrientGroup(patientNeedsProps.macronutrients, medicalConditionRecommendations, patientNeedsProps.patientProfilId),
                this.applyRecommendationToNutrientGroup(patientNeedsProps.micronutrients, medicalConditionRecommendations, patientNeedsProps.patientProfilId)
            ]);
    ...
    private async applyRecommendationToNutrientGroup(nutrientGroup: { [nutrientCode: string]: NutrientNeedsValue }, recommendations: NeedsRecommendation[], patientProfilId: AggregateID): Promise<{ [nutrientCode: string]: NutrientNeedsValue; }> {
        ...
        for (const [nutrientTagname, nutrientNeedsValue] of Object.entries(nutrientGroup)) {
            ...
            if (findedRecommendations.length > 0) {
                const contextVariableTables = findedRecommendations.reduce((contextVariableTable, recommendation) => ({ ...contextVariableTable, ...recommendation.getVariableTable() }), {});
                const context = await this.composeData(contextVariableTables, patientProfilId);
                const recommendationValue = this.needsRecommendatonPriorityManager.resolve(nutrientNeedsValue, { ...context, currentDate: new CDate() }, findedRecommendations);
