...
export class PatientNeedsModelRepositoryImpl implements PatientNeedsModelRepository {
   ...
   constructor(
      expo: SQLiteDatabase,
      private mapper: Mapper<PatientNeedsModel, PatientNeedsModelPersistence, PatientNeedsModelDto>,
   ) {
   ...
   async getById(patientNeedsModelId: AggregateID): Promise<PatientNeedsModel> {
      try {
         const modelRow = await this.db
            .select()
            .from(patientNeedsModels)
            .where(eq(patientNeedsModels.id, patientNeedsModelId as string))
            .get();
   ...
   private async checkIfExist(patientNeedsModelId: AggregateID): Promise<boolean> {
      const patientNeedsModel = await this.db
         .select()
         .from(patientNeedsModels)
         .where(eq(patientNeedsModels.id, patientNeedsModelId as string))
         .get();
