...
export class PatientProfilRepositoryImpl implements PatientProfilRepository {
    private db;
    constructor(expo: SQLiteDatabase, private mapper: Mapper<PatientProfil, PatientProfilPersistence, PatientProfilDto>, private medicalConditionRepo: MedicalConditionRepository, private objectiveRepo: ObjectiveRepository) {
        this.db = drizzle(expo)
    }
    async getById(patientProfilOrPatientId: AggregateID): Promise<PatientProfil> {
        try {
            const patientProfil = await this.db.select().from(patientProfils).where(or(eq(patientProfils.id, patientProfilOrPatientId as string), eq(patientProfils.patientId, patientProfilOrPatientId as string))).get()
    ...
    private async checkIfExist(patientProfilId: AggregateID): Promise<boolean> {
        const patientProfil = await this.db.select().from(patientProfils).where(eq(patientProfils.id, patientProfilId as string)).get()
