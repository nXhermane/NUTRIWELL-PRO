...
export class MedicalConditionRepositoryImpl implements MedicalConditionRepository {
   ...
   constructor(
      expo: SQLiteDatabase,
      private mapper: Mapper<MedicalCondition, MedicalConditionPersistence, MedicalConditionDto>,
   ) {
      this.db = drizzle(expo);
   }
   async getById(medicalConditionId: AggregateID): Promise<MedicalCondition> {
      try {
         const medicalConditionRow = await this.db
            .select()
            .from(medicalConditions)
            .where(eq(medicalConditions.id, medicalConditionId as string))
            .get();
      ...
      try {
         ...
         if (!exist) await (trx || this.db).insert(medicalConditions).values(medicalConditionPersistence);
            ...
            await (trx || this.db)
               ...
               .where(eq(medicalConditions.id, medicalConditionPersistence.id));
      } catch (error) {
   ...
   private async checkIfExist(medicalConditionId: AggregateID): Promise<boolean> {
      const medicalCondition = await this.db
         .select()
         .from(medicalConditions)
         .where(eq(medicalConditions.id, medicalConditionId as string))
         .get();
